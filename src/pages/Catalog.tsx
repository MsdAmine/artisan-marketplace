...
  // --- EFFECT 1: Load all MongoDB Products (triggered once on mount) ---
  useEffect(() => {
    setLoading(true);
...
  // --- EFFECT 2: Load and Merge Neo4j Recommendations (triggered when products change) ---
  useEffect(() => {
    async function loadRecommendations() {
      const saved = localStorage.getItem("auth");
        
      // Sortie 1: Aucune authentification
      if (!saved) {
        setRecommended([]); // Assure que les recommandations sont vides
        return;
      }

      // Sortie 2: Les produits n'ont pas encore été chargés (pour éviter la fusion sur un tableau vide)
      if (products.length === 0) return;
        
      try {
        setLoadingRecommendations(true); // Démarrage du chargement des recommandations
...
        const recommendationItems: RecommendationItem[] = data.products || []; // Assurer un tableau vide si 'products' est absent

        const productMap = new Map(products.map((p) => [p._id, p]));

        const mergedRecommendations = recommendationItems
          .map((rec) => {
            // NOTE IMPORTANTE : rec.id doit correspondre à p._id de MongoDB
...
        setRecommended([]);
      } finally {
        setLoadingRecommendations(false); // Arrêt du chargement des recommandations
      }
    }

    loadRecommendations();
  }, [products]); // Dépend de products pour se relancer après leur chargement
...
            {/* Utilisation conditionnelle de 'recommended.length > 0' pour cacher l'onglet si non pertinent */}
            <Tabs defaultValue={recommended.length > 0 ? "recommended" : "catalog"}>
...
                {/* Seulement afficher l'onglet 'Pour vous' si l'utilisateur est potentiellement connecté */}
...
                    Veuillez vous connecter pour voir vos recommandations personnalisées.
                  </div>
...
                  </div>
                ) : (
                  // Show a message if no recommendations are found after loading (but user is connected)
                  <div className="text-center py-12 text-muted-foreground">
